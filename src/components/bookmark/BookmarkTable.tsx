...
import { BookmarkFragmentFragment } from "../../../gql/graphql";

type TBookmarkTableProps = {
  bookmarks: BookmarkFragmentFragment[];
...
type TBookmarkRowProps = {
  bookmark: BookmarkFragmentFragment;
    ...
    <div className="max-w-screen-md">
      ...
      {showShareFailure && (
        <div className="toast">
          <div className="alert alert-info w-72 border border-grey-light bg-white shadow-lg text-sm text-gray-500">
            <div className="flex w-full justify-between">
              <InformationCircleIcon className="h-6 w-6 stroke-red" />
              <div>
                <span>Failed to save bookmark</span>
              </div>
              <div>
                <XMarkIcon
                  className="h-5 w-5 hover:cursor-pointer hover:bg-grey-light p-1 rounded-md"
